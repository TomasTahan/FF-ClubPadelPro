...
class ClubPadelProSupabaseUser extends BaseAuthUser {
  ClubPadelProSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> clubPadelProSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ClubPadelProSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
